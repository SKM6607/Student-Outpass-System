...
fetch(`/${CONTEXT_PATH}/studentOutpasses`, {
    ...
    .then(outpasses => {
        ...
        (
            outpass =>
                (outpass.status === STATUS_CODES.APPROVED_AND_OPEN || outpass.status === STATUS_CODES.PENDING) &&
        ...
        const [from, to] = [activeOutpass.expected_leaving_date, activeOutpass.expected_return_date]
        ...
        outpassFields.numberOfDays.textContent = String((new Date(to)).getDay() - (new Date(from)).getDay())+" Days"
